...
with loan_table as (
    ...
    from ds_overdraft.overdraft o
             join (select user_id,
                          ...
                          started_mx  as started_mx
                   from ds_overdraft.overdraft
                   group by user_id, started_date_mx, started_mx) lo
     ...
     loan_table_v2 as (
         select dl.user_id,
                ...
                case
                    when payment_amount_ > dl.max_negative * -1
                        then dl.max_negative * -1
                    when ended_mx is not null then dl.max_negative * -1
                  ...
                  left join (select *
                            ...
                            on dl.user_id = tx.user_id and
                               ...
                               least(dl.ended_mx::date, convert_timezone('America/Mexico_City'::character varying::text,
                                                                         getdate()))
         group by dl.user_id, dl.started_mx, dl.ended_mx, dl.max_negative
     ),
     ...
     od_loan_book as (
         select
             ...
             o.max_negative                                      as overdraft_loan_amount,
             payment_amount,
             case
